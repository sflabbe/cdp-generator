...
def _plot_rate_effect(
    strain_rates: np.ndarray, params, T: float, save_path: Optional[str] = None
) -> None:
    ...
    sigma_y = np.array(
        [johnson_cook_flow_stress(0.0, epsdot, T, params)[0] for epsdot in strain_rates]
    )
    ...
    ax.set_title(
        f"Strain Rate Effect on Yield Strength (T = {T:.1f} °C)", fontsize=14, fontweight="bold"
    )
...
def _plot_temperature_effect(
    temperatures: np.ndarray, params, epsdot: float, save_path: Optional[str] = None
) -> None:
    ...
    sigma_y = np.array([johnson_cook_flow_stress(0.0, epsdot, T, params)[0] for T in temperatures])
    ...
    ax.set_ylabel("Yield Strength [MPa]", fontsize=12)
    ax.set_title(
        f"Temperature Effect on Yield Strength ($\\dot{{\\varepsilon}}$ = {epsdot:.2e} 1/s)",
        fontsize=14,
        fontweight="bold",
    )
    ax.grid(True, alpha=0.3)
