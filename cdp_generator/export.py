...
def export_to_excel(results, var, mode="strain_rate", filename="CDP-Results.xlsx"):
    ...
    for i in range(len(var)):
        ...
        if is_strain_rate_mode:
            ...
            var_export_el = np.concatenate((var_export_el, np.ones(strain_length) * var[i]))
            compressive_strain_el = np.concatenate(
                (compressive_strain_el, results["compression"]["strain temp"][i])
            )
        ...
        var_export = np.concatenate(
            (var_export, np.ones(len(results["compression"]["inelastic strain"][0])) * var[i])
        )
        ...
        # For tension
        var_export_tension = np.concatenate(
            (var_export_tension, np.ones(len(results["tension"]["cracking strain"][0])) * var[i])
        )
    ...
    with pd.ExcelWriter(filename) as writer:
        compression_strain_el_df.to_excel(
